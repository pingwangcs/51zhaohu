...
foreach ($user_guids as $guid) {
	...
	if (!$user instanceof ElggUser) {
		elgg_log("ZHError , uservalidationbyemail:delete, invalid user id {$guid}"
		, "ERROR");
		$error = TRUE;
		continue;
	}
	...
	if ($is_validated !== FALSE || !$user->delete()) {
		elgg_log("ZHError , uservalidationbyemail:delete, error calling user->delete for user {$guid}"
		, "ERROR");
		$error = TRUE;
		continue;
	}
