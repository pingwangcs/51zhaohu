...
foreach ($user_guids as $guid) {
	...
	if (!$user instanceof ElggUser) {
		elgg_log("ZHError , uservalidationbyemail:resend_validation, invalid user id {$guid}"
		, "ERROR");
		$error = TRUE;
		continue;
	}
	...
	if ($is_validated !== FALSE || !uservalidationbyemail_request_validation($guid, true)) {
		elgg_log("ZHError , uservalidationbyemail, error calling  uservalidationbyemail_request_validation for user {$guid}"
		, "ERROR");
		$error = TRUE;
		continue;
	}
...
if ($error) {
	...
	elgg_log("ZHError ".$error_txt, "ERROR");
} else {
