...
function uservalidationbyemail_page_handler($page) {
	...
	if (isset($page[0]) && $page[0] == 'confirm') {
		...
		if ($code && $user) {
			if (uservalidationbyemail_validate_email($user_guid, $code)) {
				...
				try {
					...
					elgg_log("ZHError , uservalidationbyemail, user {$user_guid} , error message"
						.$e->getMessage(), "ERROR");
					
				}
			} else {
				register_error(elgg_echo('email:confirm:fail') . elgg_echo("zhaohu:sorry"));
				elgg_log("ZHError , uservalidationbyemail, error calling uservalidationbyemail_validate_email, user {$user_guid}"
				, "ERROR");
			}
		} else {
			register_error(elgg_echo('email:confirm:fail') . elgg_echo("zhaohu:sorry"));
			elgg_log("ZHError , uservalidationbyemail, error getting code and user, user {$user_guid} code {$code}"
			, "ERROR");
		}
		...
		register_error(elgg_echo('email:confirm:fail') . elgg_echo("zhaohu:sorry"));
		elgg_log("ZHError , uservalidationbyemail, error getting page[0], user {$user_guid}"
		, "ERROR");
	}
