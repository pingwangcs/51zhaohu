...
elgg.wire.viewPrevious = function(event) {
	...
	if ($link.html() == elgg.echo('wire:hide')) {
		$link.html(elgg.echo('wire:previous'));
		$link.attr("title", elgg.echo('wire:previous:help'));
		$previousDiv.slideUp(400);
	} else {
		$link.html(elgg.echo('wire:hide'));
		$link.attr("title", elgg.echo('wire:hide:help'));
...
elgg.wire.reply = function(e) {
	...
	if ($link.html() == elgg.echo('cancel')) {
		$link.html(elgg.echo('wire:reply'));
		$replyDiv.slideUp(400);
	} else {
		$link.html(elgg.echo('cancel'));
		$.ajax({type: "GET",
