 ...
 * Nathan Koterba
 */
define("WIRE_POST_LIMIT", 20);

elgg_register_event_handler('init', 'system', 'wire_init');
 ...
 *
 * Supports:
 * wire/all                  View site wire posts
 * wire/owner/<username>     View this user's wire posts
 * wire/following/<username> View the posts of those this user follows
 * wire/reply/<guid>         Reply to a post
 * wire/view/<guid>          View a post
 * wire/thread/<id>          View a conversation thread
 * wire/tag/<tag>            View wire posts tagged with <tag>
 *
 * @param array $page From the page_handler function
 * @return bool
 */
function wire_page_handler($page) {

	$base_dir = elgg_get_plugins_path() . 'wire/pages/wire';
 ...
 * 
 * @param ElggObject $wirepost Wire post object
 */
function wire_url($wirepost) {
	global $CONFIG;
	return $CONFIG->url . "wire/view/" . $wirepost->guid;
}
 ...
 * @return $string
 */
function wire_notify_message($hook, $entity_type, $returnvalue, $params) {
	global $CONFIG;
	
	$entity = $params['entity'];
	$orig_entity = $params['orig_entity'];
	if (($entity instanceof ElggEntity) && ($entity->getSubtype() == 'wire')) {
		...
		if ($entity->reply) {
			...
			$body = sprintf(elgg_echo('wire:notify:reply'), $owner->name, $parent_owner->name);
		} else {
			$body = sprintf(elgg_echo('wire:notify:post'), $owner->name);
		}
		$body .= "\n\n" . $descr . "\n\n";
		
		//$body .= elgg_echo('wire') . ": {$url}";
		$body .= elgg_echo('wire:notify:source', array($orig_entity->getURL(), $orig_entity->title));
		return $body;
	}
	return $returnvalue;
}
 ...
 * @return array
 */
function wire_get_hashtags($text) {
...
function wire_filter($text) {
	...
	$text = preg_replace(
				'/(^|[^\w])@([\p{L}\p{Nd}._]+)/u',
				'$1<a href="' . $CONFIG->wwwroot . 'wire/owner/$2">@$2</a>',
				$text);

	// hashtags
	$text = preg_replace(
				'/(^|[^\w])#(\w*[^\s\d!-\/:-@]+\w*)/',
				'$1<a href="' . $CONFIG->wwwroot . 'wire/tag/$2">#$2</a>',
				$text);
 ...
 * @return void
 */
function wire_send_response_notification($guid, $parent_guid, $user, $orig_entity) {
	...
	if ($parent_owner->guid != $user->guid) {
		...
		if ($send_response) {
			...
			$msg = wire_notify_message("", "", "", $params);

			notify_user(
					$parent_owner->guid,
					$user->guid,
					elgg_echo('wire:notify:subject', array($orig_entity->title)),
					$msg, null, 'site');
		}
	}
}
 ...
 */
function wire_get_parent($post_guid) {
...
/**
 * Sets up the entity menu for wire
 *
 ...
 * @return array
 */
function wire_setup_entity_menu_items($hook, $type, $value, $params) {
	...
	if (elgg_is_logged_in()) {
		$options = array(
			...
			'href' => "wire/reply/$entity->guid",
			'priority' => 150,
			'link_class' => 'wire-reply',
		);
	...
	if ($entity->reply) {
		$options = array(
			'name' => 'previous',
			'text' => elgg_echo('wire:previous'),
			'href' => "wire/previous/$entity->guid",
			'priority' => 160,
			'link_class' => 'wire-previous',
			'title' => elgg_echo('wire:previous:help'),
		);
		$value[] = ElggMenuItem::factory($options);
	}
	...
	$options = array(
		'name' => 'thread',
		'text' => elgg_echo('wire:thread'),
		'href' => "wire/thread/$entity->wire_thread",
		'priority' => 170,
	);
	$value[] = ElggMenuItem::factory($options);
 ...
 * @return array
 */
function wire_owner_block_menu($hook, $type, $return, $params) {
	if (elgg_instanceof($params['entity'], 'user')) {
		$url = "wire/owner/{$params['entity']->username}";
		$item = new ElggMenuItem('wire', elgg_echo('item:object:wire'), $url);
		$return[] = $item;
	}
 ...
 * @return array
 */
function wire_test($hook, $type, $value, $params) {
	global $CONFIG;
	$value[] = $CONFIG->pluginspath . 'wire/tests/regex.php';
	return $value;
}
