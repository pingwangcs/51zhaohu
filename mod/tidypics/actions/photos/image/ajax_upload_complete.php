...
if ($images) {
	...
	try {
		if ($batch->save()) {
			...
			elgg_log("ZHError , tidypics, error calling batch->save, album_id $album_guid, user_id "
		...
		elgg_log("ZHError , tidypics, Exception calling batch->save, album_id $album_guid, user_id "
		.elgg_get_logged_in_user_guid(). "Exception ".$e->getMessage(), "ERROR");
	}
} else {
	// @todo some sort of message to edit them manually.
	elgg_log("ZHError , tidypics, error calling elgg_get_entities_from_metadata, album_id $album_guid, user_id "
	.elgg_get_logged_in_user_guid(), "ERROR");
	exit;
}
