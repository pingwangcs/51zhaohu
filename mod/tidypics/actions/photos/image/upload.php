...
if (!$album) {
	register_error(elgg_echo('tidypics:baduploadform'). elgg_echo("zhaohu:sorry"));
	elgg_log("ZHError , tidypics:baduploadform, album_id {$guid}, user_id "
...
if (count($_FILES) == 0) {
	...
	register_error(elgg_echo('tidypics:exceedpostlimit'). elgg_echo("zhaohu:sorry"));
	elgg_log("ZHError , tidypics:image:upload, exceedpostlimit, user_id ".elgg_get_logged_in_user_guid(), "ERROR");
	forward(REFERER);
}
...
foreach ($_FILES['images']['name'] as $index => $value) {
	...
	if(strlen($name)>ZH_NAME_MAX){
		$nametoolong = true;
		$name = mb_substr($name, 0, ZH_NAME_MAX/3);
	}
...
if (count($not_uploaded) > 0) {
	if (count($uploaded_images) > 0) {
		$error = sprintf(elgg_echo("tidypics:partialuploadfailure"), count($not_uploaded) + count($uploaded_images), count($not_uploaded))  . '<br />';
	} else {
	...
	register_error($error. elgg_echo("zhaohu:sorry"));
	elgg_log("ZHError , tidypics:image:upload, album_id {$guid}, user_id "
		.elgg_get_logged_in_user_guid()." error ".$error, "ERROR");
