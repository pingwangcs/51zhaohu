 ...
 */
define("GROUPS_ALBUM_LIMIT", 16);
define("ALBUM_PHOTO_LIMIT", 16);
...
function tidypics_entity_menu_setup($hook, $type, $return, $params) {
	...
	if (elgg_instanceof($entity, 'object', 'album')) {
		...
		if (elgg_is_admin_logged_in()) {
			if ($album->featured == "y") {
				$url = "action/photos/album/feature?album_guid=". $album->getGUID()
	...
	if (elgg_instanceof($entity, 'object', 'image')) {
		...
		$options = array(
				'name' => 'back2album',
				'href' => $album->getURL(),
				'text' => elgg_echo('image:back2album'),
				'priority' => 70,
		);
		$return[] = ElggMenuItem::factory($options);
		...
		if (elgg_get_plugin_setting('download_link', 'tidypics')) {
			$options = array(
					'name' => 'download',
					'href' => "photos/download/$entity->guid",
