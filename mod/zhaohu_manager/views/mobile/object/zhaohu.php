<?php 
    ...
    elseif(elgg_in_context("recommended_zhaohu"))
    {
        ...
        // Time
        $content .= '<div><label>' . elgg_echo('zhaohu:start_time') . '</label>: ' . date(ZHAOHU_MANAGER_FORMAT_DATE, $zhaohu->start_day) . " ". date('H', $zhaohu->start_time) . ':' . date('i', $zhaohu->start_time);
        // optional end time
//         if ($zhaohu->end_ts) {
//         	$content .= '&nbsp;&nbsp;&nbsp;&nbsp;<label>' . elgg_echo('zhaohu:end_time') . '</label>: ' . date(ZHAOHU_MANAGER_FORMAT_DATE, $zhaohu->end_day) . " ". date('H', $zhaohu->end_ts) . ':' . date('i', $zhaohu->end_ts);
//         }
        $content .= '</div>';
        
        // zhaohu owner
        $author_text = elgg_echo('zhaouhu:initiated_by');
        $content .= '<div><label>' . $author_text . '</label>' . ': ' . $owner->name . '</div>';
        
        // group
//         $group_text = elgg_echo('zhaouhu:group_of');
//         $content .= '<div><label>' . $group_text . '</label>' . ': ' . $group->name .' </div>';
