...
elgg.zhaohu_manager.init = function() {
	...
	// mobile view js
	$('#zhaohu_mobile_search_button').live('click', function() {
// 		alert($('#search_location_state').val());
		updateQueryStr('tag', $('#zhaohu_mobile_search_input').val());
		updateQueryStr('city', '全部');
		updateQueryStr('state', $('#search_location_state').val());
	...
	$("#zhaohu_mobile_search_input").live("keydown", function(event){ 
		if ( event.which == 13 ) {
...
function zhaohu_find(){
	...
	if(showType =='groups'){
		...
		showDiv = '#find_result_groups_mobile';
		hideDiv = '#find_result_zhaohus_mobile';
	}
	else {
		proc = 'zhaohus/proc/zhaohus/find';
		showDiv = '#find_result_zhaohus_mobile';
		hideDiv = '#find_result_groups_mobile';
	}
	...
	$.ajax({
		 type: 'GET',
		 cache: false,
		 url: elgg.get_site_url() + proc +'?tag='+tag+'&state='+state+'&city='+city+'&past='+past+'&unfeatured_only='+unfeatured_only, // +'&offset='+offset
		 dataType: 'html',
...
// zhaohu_find_init(); //this actually works
// TODO: for some reason, the 'elgg.zhaohu_manager.init' didn't get called???
