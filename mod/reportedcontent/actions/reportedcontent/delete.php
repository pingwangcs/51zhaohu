...
if ($report->getSubtype() == "reported_content" && $report->canEdit()) {
	...
	if (!elgg_trigger_plugin_hook('reportedcontent:delete', 'system', array('report' => $report), TRUE)) {
		register_error(elgg_echo("reportedcontent:notdeleted") . elgg_echo("zhaohu:sorry"));
		elgg_log("ZHError reportedcontent:notdeleted report_id {$guid} user_id " .elgg_get_logged_in_user_guid()
		, "ERROR");
		forward(REFERER);
	}
