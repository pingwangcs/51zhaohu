<?php
    ...
    function encodeWho($who){
    	$whoArr=explode("<",$who);
    	if (sizeof($whoArr) == 2) {
    		return '"=?utf-8?B?' . base64_encode($whoArr[0]) . '?="<'.$whoArr[1];
    	} else
    		return $who;
    }
    
    function encodeWhoArr(&$value,$key){
    	$value = encodeWho($value);
    }
	...
	function html_email_handler_send_email(array $options = null){
		...
		if(!empty($options["to"]) && (!empty($options["html_message"]) || !empty($options["plaintext_message"]))){
			...
			if(!empty($options["from"])){
				$headers .= "From: " . encodeWho($options["from"]) . PHP_EOL;
			} else {
				$headers .= "From: " . encodeWho($site_from) . PHP_EOL;
			}
			...
			array_walk($options["to"], encodeWhoArr);
