...
&& !$user->isAdmin()) {
	register_error(elgg_echo("groups:cantcreate"). elgg_echo("zhaohu:sorry"));
	elgg_log("ZHError groups:cantcreate user_id " . $user->guid
	, "ERROR");
	forward(REFERER);
}
...
if ($group_guid && !$group->canEdit()) {
	register_error(elgg_echo("groups:cantedit") . elgg_echo("zhaohu:sorry"));
	elgg_log("ZHError groups:cantedit user_id " . $user->guid
	, "ERROR");
	forward(REFERER);
}
...
foreach (elgg_get_config('group') as $shortname => $valuetype) {
	...
	if ($valuetype == 'tags') {
		if (strlen($input[$shortname]) > ZH_TAGS_MAX) {
			register_error(elgg_echo('groups:tagstoolong').elgg_echo('input:max', array(ZH_TAGS_MAX/3, ZH_TAGS_MAX)));
			forward(REFERER);
		} else{
	...
	if($shortname=='description'){
		if (strlen($input[$shortname]) > ZH_DESP_MAX) {
			register_error(elgg_echo('groups:desptoolong').elgg_echo('input:max', array(ZH_DESP_MAX/3, ZH_DESP_MAX)));
			forward(REFERER);
		}
	}
	
	if($shortname=='city'){
		if (strlen($input[$shortname]) > ZH_CITY_MAX) {
			register_error(elgg_echo('groups:citytoolong').elgg_echo('input:max', array(ZH_CITY_MAX/3, ZH_CITY_MAX)));
			forward(REFERER);
		}
	}
}
...
if (strlen($input['name']) > ZH_NAME_MAX) {
	register_error(elgg_echo('groups:nametoolong').elgg_echo('input:max', array(ZH_NAME_MAX/3, ZH_NAME_MAX)));
	forward(REFERER);
}
...
if (!$is_new_group && $new_owner_guid && $new_owner_guid != $old_owner_guid) {
	...
	if (is_group_member($group_guid, $new_owner_guid) && ($old_owner_guid == $user->guid || $user->isAdmin())) {
		...
		$body = '<div style="color:#333;font-size:16px;">' . elgg_echo('zhgroups:transfer:onwer:body', array(
				$new_owner->name,
				$user->getURL(),
				$user->name,
				$group->getURL(),
