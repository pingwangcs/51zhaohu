...
if (($user instanceof ElggUser) && ($group instanceof ElggGroup)) {
	...
	if ($join) {
		if (groups_join_group($group, $user)) {
			...
			elgg_log("ZHError groups:cantjoin group_id {$group_guid} , user_id {$user_guid}"
			, "ERROR");
		}
	} else {
		...
		if (notify_user($group->owner_guid, $user->getGUID(), $subject, $body)) {
			...
			register_error(elgg_echo("groups:joinrequestnotmade") . elgg_echo("zhaohu:sorry"));
			elgg_log("ZHError groups:joinrequestnotmade group_id {$group_guid} , user_id {$user_guid}"
			, "ERROR");
		}
	}
} else {
