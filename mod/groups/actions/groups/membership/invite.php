...
if (count($user_guids) > 0 && elgg_instanceof($group, 'group') && $group->canEdit()) {
	foreach ($user_guids as $guid) {
		...
		if ($result) {
			...
			register_error(elgg_echo("groups:usernotinvited"). elgg_echo("zhaohu:sorry"));
			elgg_log("ZHError groups:usernotinvited group_id {$group_guid}, invited_user_id {$guid}, inviter_user_id "
