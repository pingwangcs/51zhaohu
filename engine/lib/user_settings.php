	...
	if ($user && $password) {
		...
		if ($result) {
			if ($password == $password2) {
				...
				if ($user->save ()) {
					if (smf_setUserPassword ( $user->username, $password )) {
						system_message ( elgg_echo ( 'user:password:success' ) );						
					} else {
						register_error ( elgg_echo ( 'user:bbspassword:fail' ) . elgg_echo ( 'zhaohu:sorry' ) );
						elgg_log ( "ZHError failed to set BBS password for user {$user->guid}", "ERROR" );
					}
					return true;
				} else {
					register_error ( elgg_echo ( 'user:password:fail' ) . elgg_echo ( 'zhaohu:sorry' ) );
					elgg_log ( "ZHError failed to set password for user {$user->guid}", "ERROR" );
				}
			} else {
...
function elgg_set_user_name() {
	...
	if (($user) && ($user->canEdit()) && ($name)) {
		if ($name != $user->name) {
			$user->name = $name;
			if ($user->save()) {
				if (smf_updateMemberData ( $user->username, array('real_name' => $name))) {
					system_message ( elgg_echo ( 'user:name:success' ) );
				} else {
					register_error ( elgg_echo ( 'bbsname:save:fail' ) . elgg_echo ( 'zhaohu:sorry' ) );
					elgg_log ( "ZHError failed to set BBS real name for user {$user->guid}", "ERROR" );
				}
				return true;
			} else {
				register_error(elgg_echo('user:name:fail').elgg_echo('zhaohu:sorry'));
				elgg_log("ZHError failed to set name for user {$user->guid}", "ERROR");
			}
		} else {
			// no change
			return null;
		...
		if (strcmp($email, $user->email) != 0) {
			if (!get_user_by_email($email)) {
				if ($user->email != $email) {
					...
					$user->email = $email;
					if ($user->save()) {
						if (smf_updateMemberData ( $user->username, array('email_address' => $email))) {
							system_message ( elgg_echo ( 'email:save:success' ) );							
						} else {
							register_error ( elgg_echo ( 'bbsemail:save:fail' ) . elgg_echo ( 'zhaohu:sorry' ) );
							elgg_log ( "ZHError failed to set BBS email for user {$user->guid}", "ERROR" );
						}
						return true;
					} else {
						register_error(elgg_echo('email:save:fail').elgg_echo('zhaohu:sorry'));
						elgg_log("ZHError failed to set email for user {$user->guid}", "ERROR");
					}
				}
			} else {
