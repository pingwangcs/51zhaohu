...
foreach ($profile_fields as $shortname => $valuetype) {
	...
	if ($shortname=='zip' && !empty($value)){
		if(!preg_match("/^\d{5}(-\d{4})?$/", $value)){
			register_error(elgg_echo('profile:zip:err'));
			forward(REFERER);
		}
	}
...
if ($name) {
	if (elgg_strlen($name) > 50) {
		...
		$owner->name = $name;
		$owner->save();
		if (!smf_updateMemberData ( $owner->username, array('real_name' => $name))) {
			register_error ( elgg_echo ( 'bbsname:save:fail' ) . elgg_echo ( 'zhaohu:sorry' ) );
			elgg_log ( "ZHError failed to set BBS real name for user {$owner->guid}", "ERROR" );
		}
	}
}
	...
	foreach ($input as $shortname => $value) {
		...
		if (!is_null($value) && ($value !== '')) {
			...
			if (isset($accesslevel[$shortname])) {
				...
				$access_id = 1;//corechange//ACCESS_DEFAULT
			}
			if ($shortname == "birthYear" || $shortname == "birthMonth" )
				$access_id = (int) $accesslevel["birthDay"];
			if (is_array($value)) {
				$i = 0;
