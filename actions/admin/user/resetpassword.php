...
if (($user instanceof ElggUser) && ($user->canEdit())) {
	...
	if ($user->save()) {
		...
		zhgroups_send_email($site_name, $user->email,
			elgg_echo('email:resetpassword:subject', array($site_name)),
			'<div style="color:#333;font-size:16px;">' 
			.elgg_echo('email:resetpassword:body', array($user->username, $password)).'</div>');
	} else {
